...
BaseOptions loadBaseOptions(const ros::NodeHandle &pnh, bool forward_default) {
  ...
  o.trace_dir = vk::param<std::string>(pnh, "trace_dir",
  ...
  if (vk::param<std::string>(pnh, "kfselect_criterion", default_kf_criterion) ==
      "FORWARD") {
    o.kfselect_criterion = KeyframeCriterion::FORWARD;
  } else if (vk::param<std::string>(pnh, "kfselect_criterion",
                                    default_kf_criterion) == "DOWNLOOKING") {
    o.kfselect_criterion = KeyframeCriterion::DOWNLOOKING;
  } else if (vk::param<std::string>(pnh, "kfselect_criterion",
                                    default_kf_criterion) == "LEARNT") {
    o.kfselect_criterion = KeyframeCriterion::LEARNT;
  } else {
    o.kfselect_criterion = KeyframeCriterion::FORWARD;
  }
...
GlobalMapPtr getGlobalMap(const ros::NodeHandle &pnh,
  ...
  o.relative_min_common =
      static_cast<size_t>(vk::param<int>(pnh, "gm_relative_min_common_obs", 6));
