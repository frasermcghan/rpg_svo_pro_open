...
FrameHandlerMono::FrameHandlerMono(
    ...
    const FeatureTrackerOptions &tracker_options, const CameraBundle::Ptr &cam)
...
UpdateResult FrameHandlerMono::processFrameBundle(const bool &kf_decision) {
...
void FrameHandlerMono::addImage(const cv::Mat &img, const bool &kf_decision,
                                const uint64_t timestamp) {
  addImageBundle({img}, kf_decision, timestamp);
}
...
UpdateResult FrameHandlerMono::processFrame(const bool &kf_decision) {
  ...
  if (options_.kfselect_criterion == KeyframeCriterion::LEARNT) {
    if (kf_decision == false) {
      return UpdateResult::kDefault;
    }
  } else {
...
UpdateResult
FrameHandlerMono::relocalizeFrame(const Transformation & /*T_cur_ref*/,
  ...
  const bool kf_decision_reloc = true;
  UpdateResult res = processFrame(kf_decision_reloc);
