...
bool FrameHandlerBase::addImageBundle(const std::vector<cv::Mat> &imgs,
  ...
  return addFrameBundle(frame_bundle, kf_decision);
}
...
bool FrameHandlerBase::addFrameBundle(const FrameBundlePtr &frame_bundle,
  ...
  // estimate for the CeresBackend this value should always be obtainable
  if (bundle_adjustment_) {
    ...
    bundle_adjustment_->loadMapFromBundleAdjustment(new_frames_, last_frames_,
                                                    map_, have_motion_prior_);

    if (bundle_adjustment_->getNumFrames() > 0 && have_motion_prior_) {
      bundle_adjustment_->getLatestSpeedBiasPose(&speed_bias_backend_latest_,
                                                 &T_WS_backend_latest_,
                                                 &timestamp_backend_latest_);
  ...
  if (have_motion_prior_) {
    ...
    if (!backend_scale_initialized_) {
      if (backend_scale_stable) {
        ...
        frame_utils::getSceneDepth(last_frames_->frames_[0], depth_median_,
  ...
  // Perform tracking.
  update_res_ = processFrameBundle(kf_decision);

  // We start the backend first, since it is the most time crirical
  if (bundle_adjustment_) {
#ifdef SVO_LOOP_CLOSING
    // if we have loop closing module, see whether there is any correction we
    // can do
    if (lc_) {
  ...
  if (update_res_ == UpdateResult::kKeyframe) {
    ...
    if (lc_ && last_rm_kf) {
      ...
      // update the keyframe information and trigger pose graph optimization if
      // needed
...
void FrameHandlerBase::setInitialPose(
  ...
  if (have_rotation_prior_) {
    ...
    VLOG(40) << "Set initial pose: Use inertial measurements in frame to get "
                "gravity.";
    const Vector3d g =
...
size_t FrameHandlerBase::sparseImageAlignment() {
  ...
  // previous frame best this will improve the relative transformation between
  // the previous and the new frame the result is the number of feature points
  // which could be tracked this is a hierarchical KLT solver
...
#ifdef SVO_GLOBAL_MAP
    if (!isInRecovery() && cur_reprojector->options_.use_kfs_from_global_map &&
        global_map_) {
      std::vector<FramePtr> overlap_kfs_global;
      global_map_->getOverlapKeyframesMaxN(
          *new_frames_->at(camera_idx),
          cur_reprojector->options_.max_n_global_kfs, &overlap_kfs_global);
      overlap_kfs_.at(camera_idx)
...
size_t FrameHandlerBase::optimizePose() {
  ...
  // feature world coordinates is not far off the position of the feature points
  // within the frame. The optimization is done for all points in the same time,
  // hence optimizing frame pose.
  if (options_.trace_statistics) {
    ...
    if (max_n_pts > 0) {
      ...
      std::nth_element(pts.begin(), pts.begin() + max_n_pts, pts.end(),
                       [](const PointPtr &lhs, const PointPtr &rhs) {
                         // we favour points that have not been optimized in a
                         // while
          ...
          if (it->frame_id == kf_id) {
            if (FramePtr kf = it->frame.lock()) {
              disparities.push_back(
                  (frame->px_vec_.col(i) - kf->px_vec_.col(it->keypoint_index_))
                      .norm());
            }
            break;
          }
...
void FrameHandlerBase::getMotionPrior(const bool /*use_velocity_in_frame*/) {
  ...
  /*
   ...
   imu_measurements)) return false;

   PreintegratedImuMeasurement preint(
   //      last_frames_->omega_bias_, last_frames_->omega_bias_); TODO: check
   why this worked before imuHandler()->getGyroscopeBias(),
   imuHandler()->getAccelerometerBias());
   ...
   last_frames_->getTimestampSec(), new_frames_->getTimestampSec(), false,
   R_lastimu_newimu); T_newimu_lastimu_prior_ = Transformation(R_lastimu_newimu,
   t_last_new).inverse(); return true;
   }
   */
