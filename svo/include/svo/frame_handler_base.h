...
enum class KeyframeCriterion { DOWNLOOKING, FORWARD, LEARNT };
...
public:
  ...
  bool addImageBundle(const std::vector<cv::Mat> &imgs, const bool &kf_decision,
...
protected:
  ...
  bool set_reset_; //!< Flag that the user can set. Will reset the system before
                   //!< the next iteration.
  ...
  void optimizeStructure(const FrameBundlePtr &frames, int max_n_pts,
                         int max_iter);
  ...
  virtual UpdateResult processFrameBundle(const bool &kf_decision) = 0;
